...
function loginView(){
  app.innerHTML = `<main class="auth"><div class="authbox"><div class="brand">Poker <b>Study</b><small>V10.1.11 • TEAM READY</small></div>
...
function shell(){
  app.innerHTML=`<div class="app"><aside class="sidebar"><div class="brand">Poker <b>Study</b><small>V10.1.11 • TEAM READY</small></div><nav class="nav">
...
function reloadMakeupHtml(){
  ...
  const ledgerHtml=reloadData.ledger.slice(0,24).map(e=>`<div class="finance-ledger-line ${financeEntryReversed(e)?'is-reversed':''} ${e.entry_type==='reversal'?'is-reversal':''}"><span>${reloadDay(e.created_at)}</span><b>${esc(memberName(e.player_id))}</b><strong>${esc(financeTypeLabel(e.entry_type))}${financeEntryReversed(e)?' <i class="finance-reversed-badge">ESTORNADO</i>':''}</strong><em>${esc(e.note||'')} ${+e.bank_effect?`· Banco ${signedBrl(e.bank_effect)}`:''} ${+e.frozen_makeup_effect?`· MU congelado ${signedBrl(e.frozen_makeup_effect)}`:''}</em>${canOfferFinanceReversal(e)?`<button class="btn tiny finance-reverse-btn" data-fin-reverse="${e.id}">↶ Estornar</button>`:''}</div>`).join('')||'<p class="muted">Nenhum movimento financeiro registrado.</p>'
