...
function loginView(){
  app.innerHTML = `<main class="auth"><div class="authbox"><div class="brand">Poker <b>Study</b><small>V9.8 • TRACKER</small></div>
...
function shell(){
  app.innerHTML=`<div class="app"><aside class="sidebar"><div class="brand">Poker <b>Study</b><small>V9.8 • TRACKER</small></div><nav class="nav">
...
function studySessionHtml(session){
  ...
  return `<section class="panel study-live-session guided"><header><div><span class="study-live-kicker">▶ SESSÃO GUIADA EM ANDAMENTO · V9.8</span><h2>${esc(session.title)}</h2><p class="muted">${esc(session.kind)} · bloco sugerido ${session.mins} min · iniciado ${new Date(session.startedAt).toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'})}</p></div><div class="study-live-clock" id="studyLiveClock">${String(mins).padStart(2,'0')}:${String(secs).padStart(2,'0')}</div></header>
...
async function completeActiveStudySession(){
  ...
  const row={user_id:user.id,course:'Poker Study · Plano',title:`Sessão: ${session.title}`,teacher:'Poker Study',topic:session.title,date:today(),duration,status:'done',tags:['workflow','plano',session.title,`reviewed:${reviewed.length}`].join(','),notes:[notes,...Object.entries(session.handNotes||{}).filter(([,v])=>String(v).trim()).map(([id,v])=>`Aprendizado ${id}: ${String(v).trim()}`),related.length?`Mãos relacionadas: ${related.join(', ')}`:'',reviewed.length?`Mãos revisadas: ${reviewed.join(', ')}`:''].filter(Boolean).join('\n\n')}
