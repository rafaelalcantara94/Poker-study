...
function loginView(){
  app.innerHTML = `<main class="auth"><div class="authbox"><div class="brand">Poker <b>Study</b><small>V10.1.6 • TEAM READY</small></div>
...
function shell(){
  app.innerHTML=`<div class="app"><aside class="sidebar"><div class="brand">Poker <b>Study</b><small>V10.1.6 • TEAM READY</small></div><nav class="nav">
...
function openFinanceWithdrawal(playerId){const p=profileFor(playerId);openModal('Saque / distribuição · '+memberName(playerId),`<p class="muted">Informe o lucro total disponível para distribuir. A prévia aplica automaticamente o split, Banco e, quando necessário, o regime congelado.</p><div class="form"><label>Valor total disponível<input id="fw_total" inputmode="decimal" placeholder="0,00"></label><label>Observação<input id="fw_note" placeholder="Ex.: fechamento setembro"></label><div id="fw_preview" class="finance-op-preview">Informe um valor.</div><label id="fw_waive_wrap" class="finance-bank-exception" style="display:none"><input id="fw_waive_bank" type="checkbox"> <span><b>Exceção do gestor:</b> não aplicar Banco no excedente que entrar no regime normal nesta operação.</span></label><button class="btn" id="fw_confirm">Confirmar operação</button></div>`);const inp=document.getElementById('fw_total'),prev=document.getElementById('fw_preview'),confirmBtn=document.getElementById('fw_confirm'),waiveWrap=document.getElementById('fw_waive_wrap'),waive=document.getElementById('fw_waive_bank');const upd=()=>{const v=parseReloadAmount(inp.value);if(!v){prev.innerHTML='Informe um valor.';waiveWrap.style.display='none';confirmBtn.disabled=false;return}const x=financePreview(playerId,v,waive.checked);confirmBtn.disabled=false;const active=makeupBalanceFor(playerId),frozen=frozenMakeupFor(playerId),bankBefore=bankBalanceFor(playerId);waiveWrap.style.display=x.regime==='frozen'&&x.overflow>0&&p.bank_enabled?'flex':'none';prev.innerHTML=x.regime==='normal'?`<b>Regime normal</b><span>Parte bruta jogador: ${brl(x.playerGross)}</span><span>Banco: ${brl(x.bank)}</span><span><strong>Jogador recebe: ${brl(x.playerNet)}</strong></span><span>Time: ${brl(x.team)}</span><span class="finance-preview-divider"></span><span>Make Up ativo: <strong>${brl(active)} → ${brl(active)}</strong> <small>(sem alteração)</small></span><span>Banco acumulado: <strong>${brl(bankBefore)} → ${brl(bankBefore+x.bank)}</strong></span>`:`<b>Make Up congelado</b><span>Limite pessoal restante no mês: ${brl(x.remain)}</span>${x.paydown>0?`<span>Amortização do MU congelado: <strong>${brl(x.paydown)}</strong></span>`:''}${x.overflow>0?`<span class="finance-preview-divider"></span><b>Excedente após quitar o Make Up: ${brl(x.overflow)}</b><span>O excedente entra automaticamente no regime normal 50/50.</span><span>Parte bruta jogador no excedente: ${brl(x.overflowPlayerGross)}</span><span>Banco no excedente: ${brl(x.overflowBank)}${x.waiveOverflowBank?' <small>(exceção aplicada)</small>':''}</span><span>Time no excedente: ${brl(x.overflowTeam)}</span>`:''}<span><strong>Jogador recebe no total: ${brl(x.playerNet)}</strong></span><span>Time recebe no total: ${brl(x.team)}</span><span class="finance-preview-divider"></span><span>Make Up ativo: <strong>${brl(active)} → ${brl(active)}</strong> <small>(sem alteração)</small></span><span>Make Up congelado: <strong>${brl(frozen)} → ${brl(Math.max(0,frozen-x.paydown))}</strong></span><span>Banco acumulado: <strong>${brl(bankBefore)} → ${brl(bankBefore+x.bank)}</strong>${x.bank===0?' <small>(sem alteração)</small>':''}</span>${x.autoNormal?`<span class="good"><b>✓ Ao confirmar, o perfil passa automaticamente para Regime normal.</b></span>`:''}`};inp.oninput=upd;waive.onchange=upd;confirmBtn.onclick=async()=>{const total=parseReloadAmount(inp.value);if(!total)return alert('Informe o valor.');const {teamId}=await ensureTeamContext();const {data,error}=await supabase.rpc('record_profit_distribution_v2',{p_team:teamId,p_player:playerId,p_total_profit:total,p_note:document.getElementById('fw_note').value.trim()||null,p_waive_overflow_bank:waive.checked});if(error)return alert(error.message);modal.classList.remove('show');await fetchReloadData();renderReloadPage();alert(`✓ Operação registrada. Jogador: ${brl(data.player_net||0)} · Time: ${brl(data.team_amount||0)}${+(data.bank_deposit||0)?` · Banco: ${brl(data.bank_deposit)}`:''}${+(data.frozen_paydown||0)?` · MU congelado abatido: ${brl(data.frozen_paydown)}`:''}${+(data.overflow_normal||0)?` · Excedente normal: ${brl(data.overflow_normal)}`:''}`)}}
