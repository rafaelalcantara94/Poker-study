...
function loginView(){
  app.innerHTML = `<main class="auth"><div class="authbox"><div class="brand">Poker <b>Study</b><small>V10.1.8 • TEAM READY</small></div>
...
function shell(){
  app.innerHTML=`<div class="app"><aside class="sidebar"><div class="brand">Poker <b>Study</b><small>V10.1.8 • TEAM READY</small></div><nav class="nav">
...
function reloadFinanceHtml(){
  ...
  const lsum=(field,filter=()=>true)=>ledger.filter(filter).reduce((a,e)=>a+(+e[field]||0),0)
...
function financePreview(playerId,total,waiveOverflowBank=false){const p=profileFor(playerId),split=(+p.player_split||0)+(+p.team_split||0),pr=split?+p.player_split/split:0.5,tr=split?+p.team_split/split:0.5;if(p.regime==='frozen'){const used=frozenMonthUsed(playerId),limit=+p.monthly_player_withdrawal_limit||0,remain=Math.max(0,limit-used);const pool=pr>0?Math.min(total,remain/pr):0,basePlayer=pool*pr,baseTeam=pool*tr,excess=Math.max(0,total-pool),frozen=frozenMakeupFor(playerId),pay=Math.min(excess,frozen),overflow=Math.max(0,excess-pay),overflowPlayerGross=overflow*pr,overflowTeam=overflow*tr,overflowBank=(!waiveOverflowBank)?overflowPlayerGross*(+p.bank_rate||0)/100:0,overflowPlayerNet=overflowPlayerGross-overflowBank;return {regime:'frozen',blocked:false,playerGross:basePlayer+overflowPlayerGross,playerNet:basePlayer+overflowPlayerNet,team:baseTeam+overflowTeam,bank:overflowBank,paydown:pay,overflow,overflowPlayerGross,overflowPlayerNet,overflowTeam,overflowBank,waiveOverflowBank:!!waiveOverflowBank,remain,autoNormal:frozen>0&&pay>=frozen}}const pg=total*pr,ta=total*tr,bank=p.bank_enabled?pg*(+p.bank_rate||0)/100:0;return {regime:'normal',blocked:false,playerGross:pg,playerNet:pg-bank,team:ta,bank,paydown:0,overflow:0}}
