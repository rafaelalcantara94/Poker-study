...
function loginView(){
  app.innerHTML = `<main class="auth"><div class="authbox"><div class="brand">Poker <b>Study</b><small>V9.1 • TRACKER</small></div>
...
function shell(){
  app.innerHTML=`<div class="app"><aside class="sidebar"><div class="brand">Poker <b>Study</b><small>V9.1 • TRACKER</small></div><nav class="nav">
...
function hhStatsViewHtml(facts,totalFacts=hhStatsCache){
  ...
  return `<div class="v7-dashboard">
    <div class="v7-resultbar"><b>${facts.length.toLocaleString('pt-BR')} mãos encontradas</b><span>${breakdown}</span><em>Painel V9.1: Study Workflow + Performance</em></div>
...
function saveStudyReview(handId,status,note=''){
  ...
  all[handId]={status,note:String(note||'').trim(),updatedAt:new Date().toISOString(),context:hhReplayContext?.label||all[handId]?.context||'',source:replayState.sourceName||all[handId]?.source||'',destination:studyDestinationLabel(status),heroCards:h?.heroCards||all[handId]?.heroCards||[],position:h?.positionMap?.[h.hero]||all[handId]?.position||'',stackBb:stackBb??all[handId]?.stackBb??null,tournament:h?.tournamentName||all[handId]?.tournament||'',dateTime:h?.dateTime||all[handId]?.dateTime||''};localStorage.setItem(STUDY_REVIEW_KEY,JSON.stringify(all))
}
...
function studyWorkflowHtml(h){
  ...
  const dest=r.status?`Destino local: ${studyDestinationLabel(r.status)}`:'As revisões desta V9.1 ficam neste navegador'
...
function reviews(){
  ...
  return `<div class="review-box-summary"><div><h2>📥 Caixa de Revisões</h2><p>Suas classificações do Replayer reunidas em um só lugar. Nada é enviado automaticamente para Central de Leaks ou Plano de Estudos.</p></div><div class="review-box-kpis"><span><b>${items.length}</b> revisões</span><span><b>${counts.doubt}</b> dúvidas</span><span><b>${counts.leak}</b> possíveis leaks</span><span><b>${counts.theory}</b> teoria</span></div></div><div class="review-box-tabs">${[['attention','Pendências'],['doubt','Dúvidas'],['leak','Possíveis leaks'],['theory','Rever teoria'],['correct','Corretas'],['all','Todas']].map(([k,l])=>`<button class="${active===k?'active':''}" data-review-filter="${k}">${l}</button>`).join('')}</div><div class="review-box-note"><b>Fluxo V9.1:</b> classifique no Replayer → acompanhe aqui → depois confirme o destino. Possível leak sugere Central de Leaks; Rever teoria sugere Plano de Estudos; Dúvida permanece nesta fila.</div><section class="review-box-list">${filtered.length?filtered.map(reviewCardHtml).join(''):'<div class="panel"><p class="muted">Nenhuma revisão neste filtro.</p></div>'}</section>`
}
function bindReviews(){
  ...
  document.querySelectorAll('[data-review-open]').forEach(b=>b.onclick=()=>{const id=b.dataset.reviewOpen;if(!reviewCanOpen(id))return;currentPage='replayer';route('replayer');setTimeout(()=>selectReplayHand(id),0)})
}
