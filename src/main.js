...
function loginView(){
  app.innerHTML = `<main class="auth"><div class="authbox"><div class="brand">Poker <b>Study</b><small>V9.1.1 • TRACKER</small></div>
...
function shell(){
  app.innerHTML=`<div class="app"><aside class="sidebar"><div class="brand">Poker <b>Study</b><small>V9.1.1 • TRACKER</small></div><nav class="nav">
...
function hhStatsViewHtml(facts,totalFacts=hhStatsCache){
  ...
  return `<div class="v7-dashboard">
    <div class="v7-resultbar"><b>${facts.length.toLocaleString('pt-BR')} mãos encontradas</b><span>${breakdown}</span><em>Painel V9.1.1: Study Workflow + Performance</em></div>
...
function studyWorkflowHtml(h){
  ...
  const dest=r.status?`Destino local: ${studyDestinationLabel(r.status)}`:'As revisões desta V9.1.1 ficam neste navegador'
...
function reviewCardHtml(x){
  ...
  const send=x.status==='leak'?`<button class="btn small" data-review-send="leaks" data-review-id="${esc(x.handId)}" ${x.sentTo==='leaks'?'disabled':''}>${x.sentTo==='leaks'?'✓ Na Central de Leaks':'🧠 Enviar para Leaks'}</button>`:x.status==='theory'?`<button class="btn small" data-review-send="plan" data-review-id="${esc(x.handId)}" ${x.sentTo==='plan'?'disabled':''}>${x.sentTo==='plan'?'✓ No Plano de Estudos':'🗓️ Enviar para Plano'}</button>`:''
  return `<article class="review-box-card ${esc(x.status)}"><div class="review-box-icon">${studyStatusIcon(x.status)}</div><div class="review-box-main"><div class="review-box-title"><b>${esc(cards)}</b><span>${esc(x.position||'')}${x.stackBb!=null?` · ${esc(String(x.stackBb))}bb`:''}</span></div><p>${esc(x.note||'Sem nota')}</p><small>${esc(x.context||x.source||'Revisão do Replayer')}${when?` · ${esc(when)}`:''}${legacy?' · legado V9.0.1':''}</small>${x.sentTo?`<small class="good">${reviewDestinationText(x)}</small>`:''}</div><div class="review-box-actions"><span class="review-box-status ${esc(x.status)}">${esc(studyStatusLabel(x.status))}</span><button class="btn small secondary" data-review-open="${esc(x.handId)}" ${can?'':'disabled'}>${can?'🎬 Abrir mão':'Fonte não carregada'}</button>${send}</div></article>`
}
function reviews(){
  const items=reviewBoxItems(),counts={correct:0,doubt:0,leak:0,theory:0};items.forEach(x=>counts[x.status]=(counts[x.status]||0)+1)
  const active=window.reviewBoxFilter||'attention',filtered=active==='all'?items:active==='attention'?items.filter(x=>x.status!=='correct'):items.filter(x=>x.status===active)
  return `<div class="review-box-summary"><div><h2>📥 Caixa de Revisões</h2><p>Suas classificações do Replayer reunidas em um só lugar. O destino só é confirmado quando você clicar em enviar.</p></div><div class="review-box-kpis"><span><b>${items.length}</b> revisões</span><span><b>${counts.doubt}</b> dúvidas</span><span><b>${counts.leak}</b> possíveis leaks</span><span><b>${counts.theory}</b> teoria</span></div></div><div class="review-box-tabs">${[['attention','Pendências'],['doubt','Dúvidas'],['leak','Possíveis leaks'],['theory','Rever teoria'],['correct','Corretas'],['all','Todas']].map(([k,l])=>`<button class="${active===k?'active':''}" data-review-filter="${k}">${l}</button>`).join('')}</div><div class="review-box-note"><b>Fluxo V9.1.1:</b> novas classificações guardam um snapshot compacto da mão, então podem ser reabertas mesmo depois de sair da sessão. Revisões antigas sem snapshot continuam identificadas como legado.</div><section class="review-box-list">${filtered.length?filtered.map(reviewCardHtml).join(''):'<div class="panel"><p class="muted">Nenhuma revisão neste filtro.</p></div>'}</section>`
}
...
function sendReviewDestination(id,dest){
  ...
  const label=dest==='leaks'?'Central de Leaks':'Plano de Estudos';if(!confirm(`Enviar esta revisão para ${label}?`))return
  r.sentTo=dest;r.sentAt=new Date().toISOString();all[id]=r;localStorage.setItem(STUDY_REVIEW_KEY,JSON.stringify(all));studyToast(`✓ Enviado para ${label}`);page.innerHTML=reviews();bindReviews()
}
function bindReviews(){
  document.querySelectorAll('[data-review-filter]').forEach(b=>b.onclick=()=>{window.reviewBoxFilter=b.dataset.reviewFilter;page.innerHTML=reviews();bindReviews()})
