...
function loginView(){
  app.innerHTML = `<main class="auth"><div class="authbox"><div class="brand">Poker <b>Study</b><small>V10.1 • TEAM READY</small></div>
...
function shell(){
  app.innerHTML=`<div class="app"><aside class="sidebar"><div class="brand">Poker <b>Study</b><small>V10.1 • TEAM READY</small></div><nav class="nav">
...
function playerOwnFinanceHtml(){const rows=reloadData.items.filter(x=>x.request.player_id===user.id),sum=f=>rows.filter(f).reduce((a,x)=>a+(+x.amount||0),0),p=ownFinancialProfile(),last=latestClosing(),bank=ownLedgerBalance('bank_effect');return `<section class="panel reload-finance"><div class="reload-finance-head"><div><span class="reload-kicker">MEU RELATÓRIO FINANCEIRO</span><h2>Sua posição financeira</h2><p class="muted">Somente os seus dados aparecem aqui.</p></div></div><div class="reload-finance-kpis"><div><small>Solicitado</small><strong>${brl(sum(()=>true))}</strong></div><div class="approved"><small>Aprovado</small><strong>${brl(sum(x=>['approved','paid'].includes(x.status)))}</strong></div><div class="sent"><small>Enviado</small><strong>${brl(sum(x=>x.status==='paid'))}</strong></div><div><small>Caixa atual</small><strong>${brl(closingTotal(last))}</strong></div><div><small>Banco</small><strong>${brl(bank)}</strong></div></div><div class="finance-profile-card"><h3>Seu regime</h3><p><b>${p.regime==='frozen'?'❄ Make Up congelado':'Regime normal'}</b> · Split ${Number(p.player_split)}/${Number(p.team_split)} · Banco ${p.bank_enabled?Number(p.bank_rate)+'%':'inativo'}</p>${p.regime==='frozen'?`<p>Make Up congelado base: <b>${brl(p.frozen_makeup_balance)}</b> · Limite mensal pessoal: <b>${p.monthly_player_withdrawal_limit==null?'—':brl(p.monthly_player_withdrawal_limit)}</b></p>`:''}</div>${playerCashHistoryHtml(user.id)}</section>`}
