...
function loginView(){
  app.innerHTML = `<main class="auth"><div class="authbox"><div class="brand">Poker <b>Study</b><small>V12.6.0 • POKERSTARS + BOUNTY</small></div>
...
function shell(){
  app.innerHTML=`<div class="app"><aside class="sidebar"><div class="brand">Poker <b>Study</b><small>V12.6.0 • POKERSTARS + BOUNTY</small></div><nav class="nav">
...
function replayStageHtml(h){
  ...
  const seats=h.seats.map(p=>{
    ...
    return `<div class="seat-cards ${classes}" style="left:${left}%;top:${top}%"><div class="mini-cards">${cards.length?cards.map(cardHtml).join(''):'<span class="card-back">?</span><span class="card-back">?</span>'}</div></div><div class="seat-info ${classes}" style="left:${ip.left}%;top:${ip.top}%"><b>${esc(pos)} · ${esc(p.name)}</b><span>${stackBb.toFixed(1)}bb (${fmtFullChips(ps.stack)})</span>${p.bounty!=null?`<span class="seat-bounty">🎯 $${Number(p.bounty).toFixed(2)}</span>`:''}</div>${bet>0?`<div class="table-bet" style="left:${bp.left}%;top:${bp.top}%"><span class="chip-stack"><i></i><i></i><i></i></span><b>${fmtChips(bet)}</b><small>${h.bb?(bet/h.bb).toFixed(1)+'bb':''}</small></div>`:''}`
  }).join('')
