...
function studySessionHtml(session){
  ...
  return `<section class="panel study-live-session guided"><header><div><span class="study-live-kicker">▶ SESSÃO GUIADA EM ANDAMENTO · V9.9</span><h2>${esc(session.title)}</h2><p class="muted">${esc(session.kind)} · bloco sugerido ${session.mins} min · iniciado ${new Date(session.startedAt).toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'})}</p></div><div class="study-live-clock" id="studyLiveClock">${String(mins).padStart(2,'0')}:${String(secs).padStart(2,'0')}</div></header>
...
function studyLearningItems(){
  const out=[]
  for(const st of db.studies.filter(x=>x.status==='done')){
