...
function loginView(){
  app.innerHTML = `<main class="auth"><div class="authbox"><div class="brand">Poker <b>Study</b><small>V9.6 • TRACKER</small></div>
...
function shell(){
  app.innerHTML=`<div class="app"><aside class="sidebar"><div class="brand">Poker <b>Study</b><small>V9.6 • TRACKER</small></div><nav class="nav">
...
function dashboard(){
  ...
  <section class="panel dash-next"><header><div><h2>Próximo bloco</h2><p class="muted">O Plano de Estudos continua sendo a fonte; aqui mostramos só a próxima ação.</p></div><button class="btn small secondary" data-dash-route="plan">Ver plano completo →</button></header>${next?`<div class="dash-next-row"><div><span class="dash-eyebrow">${esc(next.kind)}</span><h3>${esc(next.title)}</h3><p>${esc(next.why)}</p></div><div><strong>${next.mins} min</strong><button class="btn" data-dash-study-start="0">▶ Iniciar estudo</button></div></div>`:'<div class="dash-empty">Nenhum bloco sugerido no momento.</div>'}</section>`
}
