...
function loginView(){
  app.innerHTML = `<main class="auth"><div class="authbox"><div class="brand">Poker <b>Study</b><small>V10.4.5 • TEAM INTELLIGENCE</small></div>
...
function shell(){
  app.innerHTML=`<div class="app"><aside class="sidebar"><div class="brand">Poker <b>Study</b><small>V10.4.5 • TEAM INTELLIGENCE</small></div><nav class="nav">
...
function bindTeamCoachPlanDelegation(){
  ...
  document.addEventListener('click',e=>{
    ...
    const topic=btn.dataset.teamCreatePlan||''
    const rows=window.__teamCoachRows||[]
    const plan=createTeamCoachPlan(topic,rows)
    if(!plan){alert('Não foi possível criar a aula. Atualize a Central do Time e tente novamente.');return}
    btn.textContent='✓ Aula criada'
    btn.disabled=true
    studyToast('✓ Aula criada no Plano de Estudos')
    setTimeout(()=>route('studyplan'),180)
  })
}
...
function teamcenterHtml(rows,allRows=rows){
  ...
  return `<div class="team-page-v3"><div class="team-page-head"><div><h1>👥 Central do Time</h1><p>Inteligência técnica, prioridades e comparação dos jogadores</p></div><div class="team-update"><span>↻</span><small>Última atualização</small><b>${esc(latestText)}</b></div></div><section class="team-hero team-hero-v3"><div class="team-hero-copy"><span class="team-kicker">V10.4.5 · STUDY WORKFLOW FIX</span><h2>Raio-X técnico da equipe</h2><p>Do diagnóstico à ação: clique, investigue e leve o spot para estudo.</p></div><div class="team-hero-quote">“O painel só tem valor<br>se levar a uma decisão.”</div></section>${teamFilterBar(allRows)}
