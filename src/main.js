...
function loginView(){
  app.innerHTML = `<main class="auth"><div class="authbox"><div class="brand">Poker <b>Study</b><small>V10.1.2 • TEAM READY</small></div>
...
function shell(){
  app.innerHTML=`<div class="app"><aside class="sidebar"><div class="brand">Poker <b>Study</b><small>V10.1.2 • TEAM READY</small></div><nav class="nav">
...
function reloadMakeupHtml(){
  ...
  return `<section class="panel reload-makeup finance-engine"><div class="reload-finance-head"><div><span class="reload-kicker">FINANCE ENGINE</span><h2>Make Up, Banco, saques e adiantamentos</h2><p class="muted">Todos os movimentos ficam no razão financeiro. O Banco pertence ao jogador e o regime congelado é controlado separadamente.</p></div></div><div class="reload-makeup-kpis"><div><small>Exposição de Make Up</small><strong>${brl(total)}</strong></div><div><small>Banco acumulado dos jogadores</small><strong>${brl(bankTotal)}</strong></div></div><div class="reload-table-wrap"><table><thead><tr><th>Jogador</th><th>Regime</th><th>Make Up ativo</th><th>Make Up congelado</th><th>Banco</th><th>Limite mensal</th><th>Ações</th></tr></thead><tbody>${arr.map(x=>`<tr><td><b>${esc(memberName(x.member.user_id))}</b><br><small>${esc(x.member.role)}</small></td><td>${x.profile.regime==='frozen'?'<span class="reload-status pending">Congelado</span>':'<span class="reload-status approved">Normal</span>'}</td><td><b>${brl(x.makeup)}</b></td><td><b>${brl(x.frozen)}</b></td><td class="good"><b>${brl(x.bank)}</b></td><td>${x.profile.regime==='frozen'?`${x.profile.monthly_player_withdrawal_limit==null?'—':brl(x.profile.monthly_player_withdrawal_limit)}<br><small>usado ${brl(x.used)}</small>`:'—'}</td><td><div class="finance-engine-actions"><button class="btn tiny secondary" data-fin-profile="${x.member.user_id}">⚙ Perfil</button><button class="btn tiny" data-fin-withdraw="${x.member.user_id}">💸 Saque</button><button class="btn tiny secondary" data-fin-advance="${x.member.user_id}">⏩ Adiant.</button><button class="btn tiny secondary" data-fin-return="${x.member.user_id}">↩ Devolução</button><button class="btn tiny secondary" data-fin-bank="${x.member.user_id}">🏦 Banco</button></div></td></tr>`).join('')}</tbody></table></div><div class="makeup-history"><h3>Livro financeiro recente</h3>${reloadData.ledger.slice(0,20).map(e=>`<div><span>${reloadDay(e.created_at)}</span><b>${esc(memberName(e.player_id))}</b><strong>${esc(financeTypeLabel(e.entry_type))}</strong><em>${esc(e.note||'')} ${+e.bank_effect?`· Banco ${e.bank_effect>0?'+':''}${brl(e.bank_effect)}`:''} ${+e.frozen_makeup_effect?`· MU congelado ${brl(e.frozen_makeup_effect)}`:''}</em></div>`).join('')||'<p class="muted">Nenhum movimento financeiro registrado.</p>'}</div></section>`
}

function financePreview(playerId,total){const p=profileFor(playerId),split=(+p.player_split||0)+(+p.team_split||0),pr=split?+p.player_split/split:0.5,tr=split?+p.team_split/split:0.5;if(p.regime==='frozen'){const used=frozenMonthUsed(playerId),limit=+p.monthly_player_withdrawal_limit||0,remain=Math.max(0,limit-used);if(remain<=0)return {regime:'frozen',blocked:true,playerGross:0,playerNet:0,team:0,bank:0,paydown:0,unallocated:total,remain:0};const pool=pr>0?Math.min(total,remain/pr):0,pg=pool*pr,ta=pool*tr,excess=Math.max(0,total-pool),frozen=frozenMakeupFor(playerId),pay=Math.min(excess,frozen);return {regime:'frozen',blocked:false,playerGross:pg,playerNet:pg,team:ta,bank:0,paydown:pay,unallocated:Math.max(0,excess-pay),remain}}const pg=total*pr,ta=total*tr,bank=p.bank_enabled?pg*(+p.bank_rate||0)/100:0;return {regime:'normal',blocked:false,playerGross:pg,playerNet:pg-bank,team:ta,bank,paydown:0,unallocated:0}}
function openFinanceProfile(playerId){const p=profileFor(playerId);openModal('Perfil financeiro · '+memberName(playerId),`<div class="form finance-profile-form"><label>Regime<select id="fp_regime"><option value="normal" ${p.regime==='normal'?'selected':''}>Normal</option><option value="frozen" ${p.regime==='frozen'?'selected':''}>Make Up congelado</option></select></label><label>Split jogador (%)<input id="fp_player" type="number" step="0.01" value="${+p.player_split||50}"></label><label>Split time (%)<input id="fp_team" type="number" step="0.01" value="${+p.team_split||50}"></label><label>Banco sobre saque (%)<input id="fp_bank_rate" type="number" step="0.01" value="${+p.bank_rate||10}"></label><label><input id="fp_bank_enabled" type="checkbox" ${p.bank_enabled?'checked':''}> Banco ativo no regime normal</label><label>Make Up congelado base<input id="fp_frozen" inputmode="decimal" value="${(+p.frozen_makeup_balance||0).toFixed(2).replace('.',',')}"></label><label>Limite mensal do jogador<input id="fp_limit" inputmode="decimal" value="${p.monthly_player_withdrawal_limit==null?'':(+p.monthly_player_withdrawal_limit).toFixed(2).replace('.',',')}"></label><button class="btn" id="saveFinanceProfile">Salvar perfil</button></div>`);document.getElementById('saveFinanceProfile').onclick=async()=>{const {teamId}=await ensureTeamContext();const payload={team_id:teamId,player_id:playerId,regime:document.getElementById('fp_regime').value,player_split:+document.getElementById('fp_player').value||0,team_split:+document.getElementById('fp_team').value||0,bank_rate:+document.getElementById('fp_bank_rate').value||0,bank_enabled:document.getElementById('fp_bank_enabled').checked,frozen_makeup_balance:parseReloadAmount(document.getElementById('fp_frozen').value),monthly_player_withdrawal_limit:document.getElementById('fp_limit').value.trim()===''?null:parseReloadAmount(document.getElementById('fp_limit').value),updated_at:new Date().toISOString()};const {error}=await supabase.from('financial_profiles').upsert(payload,{onConflict:'team_id,player_id'});if(error)return alert(error.message);modal.classList.remove('show');await fetchReloadData();renderReloadPage()}}
function openFinanceWithdrawal(playerId){const p=profileFor(playerId);openModal('Saque / distribuição · '+memberName(playerId),`<p class="muted">Informe o lucro total disponível para distribuir. A prévia aplica automaticamente o split, Banco e, quando necessário, o regime congelado.</p><div class="form"><label>Valor total disponível<input id="fw_total" inputmode="decimal" placeholder="0,00"></label><label>Observação<input id="fw_note" placeholder="Ex.: fechamento setembro"></label><div id="fw_preview" class="finance-op-preview">Informe um valor.</div><button class="btn" id="fw_confirm">Confirmar operação</button></div>`);const inp=document.getElementById('fw_total'),prev=document.getElementById('fw_preview');const confirmBtn=document.getElementById('fw_confirm');const upd=()=>{const v=parseReloadAmount(inp.value);if(!v){prev.innerHTML='Informe um valor.';confirmBtn.disabled=false;return}const x=financePreview(playerId,v);if(x.blocked){prev.innerHTML=`<b>Make Up congelado</b><span class="bad"><strong>Limite mensal de saque esgotado.</strong></span><span>Disponível para o jogador neste mês: ${brl(0)}</span><span>Use uma operação de amortização/ajuste separada caso queira apenas reduzir o Make Up congelado.</span>`;confirmBtn.disabled=true;return}confirmBtn.disabled=false;prev.innerHTML=x.regime==='normal'?`<b>Regime normal</b><span>Parte bruta jogador: ${brl(x.playerGross)}</span><span>Banco: ${brl(x.bank)}</span><span><strong>Jogador recebe: ${brl(x.playerNet)}</strong></span><span>Time: ${brl(x.team)}</span>`:`<b>Make Up congelado</b><span>Limite pessoal restante no mês: ${brl(x.remain)}</span><span><strong>Jogador recebe: ${brl(x.playerNet)}</strong></span><span>Time: ${brl(x.team)}</span><span>Amortização do MU congelado: ${brl(x.paydown)}</span>${x.unallocated>0?`<span class="bad">Excedente não alocado: ${brl(x.unallocated)}</span>`:''}`};inp.oninput=upd;confirmBtn.onclick=async()=>{const total=parseReloadAmount(inp.value);if(!total)return alert('Informe o valor.');const check=financePreview(playerId,total);if(check.blocked)return alert('Limite mensal de saque esgotado para este jogador.');const {teamId}=await ensureTeamContext();const {data,error}=await supabase.rpc('record_profit_distribution',{p_team:teamId,p_player:playerId,p_total_profit:total,p_note:document.getElementById('fw_note').value.trim()||null});if(error)return alert(error.message);modal.classList.remove('show');await fetchReloadData();renderReloadPage();alert(`✓ Operação registrada. Jogador: ${brl(data.player_net||0)} · Time: ${brl(data.team_amount||0)}${+(data.bank_deposit||0)?` · Banco: ${brl(data.bank_deposit)}`:''}${+(data.frozen_paydown||0)?` · MU congelado abatido: ${brl(data.frozen_paydown)}`:''}`)}}
...
function openFinanceReturn(playerId){openModal('Devolução / acerto · '+memberName(playerId),`<div class="form"><label>Valor devolvido ao time<input id="fr_amount" inputmode="decimal" placeholder="0,00"></label><label>Observação<input id="fr_note" placeholder="Motivo do acerto"></label><button class="btn" id="fr_confirm">Registrar devolução</button></div>`);document.getElementById('fr_confirm').onclick=async()=>{const amount=parseReloadAmount(document.getElementById('fr_amount').value);if(!amount)return alert('Informe o valor.');const {teamId}=await ensureTeamContext();const {error}=await supabase.rpc('record_team_return',{p_team:teamId,p_player:playerId,p_amount:amount,p_note:document.getElementById('fr_note').value.trim()||null});if(error)return alert(error.message);modal.classList.remove('show');await fetchReloadData();renderReloadPage()}}
