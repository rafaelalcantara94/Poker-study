...
function loginView(){
  app.innerHTML = `<main class="auth"><div class="authbox"><div class="brand">Poker <b>Study</b><small>V10.3.2 • TEAM INTELLIGENCE</small></div>
...
function shell(){
  app.innerHTML=`<div class="app"><aside class="sidebar"><div class="brand">Poker <b>Study</b><small>V10.3.2 • TEAM INTELLIGENCE</small></div><nav class="nav">
...
function teamViewSnap(snap,period='all'){
  if(!snap)return null
  const views=snap.stats?.views||{},view=views[period+'|all']||views['all|all']
  if(!view)return snap
...
function teamFilteredRows(rows){
  return rows.filter(x=>teamCenterFilters.player==='all'||x.member.user_id===teamCenterFilters.player).map(x=>({...x,snap:teamViewSnap(x.snap,teamCenterFilters.period)}))
}
function teamFilterBar(rows){
  ...
  return `<section class="team-filterbar team-filterbar-v3"><div class="team-filter-icon">👥</div><div><small>JOGADOR</small><select id="teamFilterPlayer"><option value="all">Todos os jogadores</option>${rows.map(x=>`<option value="${esc(x.member.user_id)}" ${teamCenterFilters.player===x.member.user_id?'selected':''}>${esc(x.member.display_name||x.member.email)}</option>`).join('')}</select></div><div class="team-filter-icon">🗓</div><div><small>PERÍODO</small><select id="teamFilterPeriod">${Object.entries(periodLabels).map(([k,v])=>`<option value="${k}" ${teamCenterFilters.period===k?'selected':''}>${v}</option>`).join('')}</select></div><button class="btn secondary small team-clear-btn" id="teamClearFilters">↻ Limpar filtros</button><span class="team-filter-meta">Dados agregados dos snapshots publicados. As HHs continuam privadas.</span></section>`
}
function teamPriorityScore(x){
  const leaks=x.snap?.leaks||[],hi=leaks.filter(l=>l.score>=1.5).length
  const score=leaks.slice(0,5).reduce((n,l)=>n+(+l.score||0),0)
  return hi*100+score*10+Math.min(20,Math.log10(Math.max(10,+x.snap?.hands||0))*4)
}
function teamCollectiveLeaks(ready){
  const map={}
  ready.forEach(x=>(x.snap.leaks||[]).forEach(l=>{
    const key=String(l.label||'').trim()
    if(!key)return
    if(!map[key])map[key]={label:key,players:new Set(),items:[],score:0}
    map[key].players.add(x.member.display_name||x.member.email)
    map[key].items.push({...l,player:x.member.display_name||x.member.email})
    map[key].score+=+l.score||0
  }))
...
function teamPlayerXray(row){
  const z=row.snap?.stats||{},leaks=[...(row.snap?.leaks||[])].sort((a,b)=>b.score-a.score),top=leaks.slice(0,6)
  const strengths=[
    {label:'WWSF',value:z.wwsf,good:+z.wwsf>=46,text:`${teamNum(z.wwsf)}%`},
    {label:'bb/100',value:z.bb100,good:+z.bb100>=0,text:`${+z.bb100>=0?'+':''}${teamNum(z.bb100)} bb/100`}
  ]
  const name=row.member.display_name||row.member.email,initials=name.slice(0,2).toUpperCase()
  return `<div class="team-modal-backdrop" id="teamXrayBackdrop"><section class="team-xray-modal">
    <header><div class="team-xray-avatar">${esc(initials)}</div><div><small>RAIO-X INDIVIDUAL</small><h2>${esc(name)}</h2><p>${esc(row.member.role||'player')} · ${(+row.snap.hands||0).toLocaleString('pt-BR')} mãos no recorte</p></div><button id="teamXrayClose">×</button></header>
    ...
    <div class="team-xray-grid">
      <div class="team-xray-box"><h3>🎯 Prioridades técnicas</h3>${top.length?top.map((l,i)=>`<div class="team-xray-leak"><b>${i+1}</b><span><strong>${esc(l.label)}</strong><small>${teamNum(l.value)}% · ref. ${esc(l.range)} · ${(+l.den||0).toLocaleString('pt-BR')} opp</small></span><em class="${l.score>=1.5?'critical':'important'}">${l.score>=1.5?'ALTA':'MÉDIA'}</em></div>`).join(''):'<p class="muted">Nenhum desvio prioritário neste recorte.</p>'}</div>
      <div class="team-xray-box"><h3>🧠 Leitura do gestor</h3><div class="team-coach-note"><b>${leaks.filter(l=>l.score>=1.5).length} alerta(s) de alta prioridade</b><span>${leaks[0]?`Primeiro tema recomendado: ${esc(leaks[0].label)}.`:'Sem foco crítico agora.'}</span></div><div class="team-coach-note"><b>Amostra</b><span>${(+row.snap.hands||0).toLocaleString('pt-BR')} mãos disponíveis neste recorte.</span></div><div class="team-coach-note"><b>Perfil atual</b><span>${+z.vpip>26?'Mais loose/agressivo no pré-flop':+z.vpip<22?'Mais tight no pré-flop':'Faixa intermediária de VPIP'}; 3Bet em ${teamNum(z.threeBet)}%.</span></div></div>
    </div>
  </section></div>`
}
function teamcenterHtml(rows,allRows=rows){
  const ready=rows.filter(x=>x.snap),total=ready.reduce((n,x)=>n+(+x.snap.hands||0),0)
  const allLeaks=ready.flatMap(x=>(x.snap.leaks||[]).map(l=>({...l,player:x.member.display_name||x.member.email,userId:x.member.user_id}))).sort((a,b)=>b.score-a.score)
  const critical=allLeaks.filter(x=>x.score>=1.5).length,medium=allLeaks.filter(x=>x.score>=.75&&x.score<1.5).length
  ...
  const priorities=[...ready].sort((a,b)=>teamPriorityScore(b)-teamPriorityScore(a))
  ...
  const threeBetCount=allLeaks.filter(l=>/3bet/i.test(l.label||'')).length
  const periodName=({all:'Base completa','30d':'30 dias','90d':'90 dias','180d':'6 meses','365d':'12 meses'})[teamCenterFilters.period]
  ...
  <div class="team-kpis team-kpis-v3"><button class="team-kpi-click" data-team-nav="players"><span class="team-kpi-icon">👥</span><small>JOGADORES COM DADOS</small><strong>${ready.length}/${rows.length}</strong></button><button class="team-kpi-click" data-team-nav="hhstats"><span class="team-kpi-icon green">▤</span><small>MÃOS ANALISADAS</small><strong>${total.toLocaleString('pt-BR')}</strong></button><button class="team-kpi-click" data-team-nav="hhstats"><span class="team-kpi-icon green">↗</span><small>WINRATE MÉDIO</small><strong class="${avg>=0?'good-text':'bad-text'}">${avg>=0?'+':''}${teamNum(avg)} bb/100</strong></button><button class="team-kpi-click" data-team-nav="leaks"><span class="team-kpi-icon red">!</span><small>ALERTAS DE ALTA PRIORIDADE</small><strong>${critical}</strong></button></div>
  <div class="team-dashboard-row">
    <section class="panel team-performance-card"><header class="team-section-head"><div><span class="team-section-icon gold">🏆</span><h2>Desempenho dos jogadores</h2><p class="muted">Visão geral do desempenho técnico no período selecionado.</p></div><button class="btn small" id="teamOpenTopXray">Ver análise detalhada →</button></header>
    ${ranking.length?`<div class="team-table team-table-clickable"><div class="team-tr team-th"><span># Jogador</span><span>Mãos</span><span>bb/100</span><span>VPIP</span><span>PFR</span><span>3Bet</span><span>WWSF</span><span>Alertas</span></div>${ranking.map((x,i)=>{const z=x.snap.stats||{},hi=(x.snap.leaks||[]).filter(l=>l.score>=1.5).length;return `<button class="team-tr team-player-row" data-team-player="${esc(x.member.user_id)}"><span class="team-player"><i>${esc((x.member.display_name||x.member.email||'?').slice(0,2).toUpperCase())}</i><b>${i+1}. ${esc(x.member.display_name||x.member.email)}</b><small>${esc(x.member.role||'player')}</small></span><span>${(+x.snap.hands||0).toLocaleString('pt-BR')}</span><span class="${z.bb100>=0?'good-text':'bad-text'}"><b>${z.bb100>=0?'+':''}${teamNum(z.bb100)}</b></span><span>${teamNum(z.vpip)}%</span><span>${teamNum(z.pfr)}%</span><span>${teamNum(z.threeBet)}%</span><span>${teamNum(z.wwsf)}%</span><span><b class="team-alert-count">${hi}</b></span></button>`}).join('')}</div>`:'<div class="notice">Nenhum dado encontrado neste recorte.</div>'}</section>
    <section class="panel team-alert-card team-alert-v3"><header class="team-section-head"><div><span class="team-section-icon red">⚠</span><h2>Distribuição dos alertas</h2><p class="muted">Total de ${allLeaks.length} sinais elegíveis.</p></div></header><div class="team-alert-ring" style="--high:${allLeaks.length?critical/allLeaks.length*100:0}"><div><strong>${allLeaks.length}</strong><span>alertas</span></div></div><div class="team-alert-legend"><div><i class="high"></i><span>Alta prioridade</span><b>${critical}</b></div><div><i class="medium"></i><span>Média prioridade</span><b>${medium}</b></div><div><i class="low"></i><span>Demais sinais</span><b>${Math.max(0,allLeaks.length-critical-medium)}</b></div></div></section>
  </div>
  <div class="team-dashboard-row team-row-2">
    <section class="panel team-priority-map"><header class="team-section-head"><div><span class="team-section-icon purple">↗</span><h2>Mapa de prioridades do time</h2><p class="muted">Substitui gráficos históricos até termos snapshots temporais reais.</p></div><span class="pill">${esc(playerName)} · ${esc(periodName)}</span></header><div class="team-priority-bars">${priorities.map((x,i)=>{const hi=(x.snap.leaks||[]).filter(l=>l.score>=1.5).length,score=Math.min(100,25+hi*11+(x.snap.leaks?.[0]?.score||0)*9),name=x.member.display_name||x.member.email;return `<button class="team-priority-row" data-team-player="${esc(x.member.user_id)}"><span><b>${i+1}. ${esc(name)}</b><small>${hi} alertas altos · foco: ${esc(x.snap.leaks?.[0]?.label||'sem prioridade crítica')}</small></span><div class="team-bar"><i style="width:${score}%"></i></div><em>${Math.round(score)}</em></button>`}).join('')||'<p class="muted">Sem jogadores neste filtro.</p>'}</div></section>
    <section class="panel team-leaks-v3"><header class="team-section-head"><div><span class="team-section-icon red">🎯</span><h2>Principais leaks do time</h2><p class="muted">Ordenados por prioridade e impacto técnico.</p></div><span class="pill">${allLeaks.length} sinais</span></header><div class="team-leaks">${allLeaks.slice(0,5).map((x,i)=>`<div><span class="rank">${i+1}</span><span><b>${esc(x.label)}</b><small>${esc(x.player)} · ${teamNum(x.value)}% · ref. ${esc(x.range)} · ${(+x.den||0).toLocaleString('pt-BR')} opp</small></span><em class="${x.score>=1.5?'critical':'important'}">${x.score>=1.5?'ALTA':'MÉDIA'}</em></div>`).join('')||'<p class="muted">Sem leaks neste recorte.</p>'}</div></section>
  </div>
  <section class="panel team-comparison-chart"><header class="team-section-head"><div><span class="team-section-icon blue">▥</span><h2>Comparativo técnico</h2><p class="muted">Leitura visual dos principais indicadores do recorte. Clique em um jogador para abrir o raio-X.</p></div></header><div class="team-compare-grid">${ready.map(x=>{const z=x.snap.stats||{},name=x.member.display_name||x.member.email;return `<button class="team-compare-player" data-team-player="${esc(x.member.user_id)}"><b>${esc(name)}</b><div><span>VPIP <em>${teamNum(z.vpip)}%</em></span><i><u style="width:${Math.min(100,+z.vpip||0)}%"></u></i></div><div><span>PFR <em>${teamNum(z.pfr)}%</em></span><i><u style="width:${Math.min(100,(+z.pfr||0)*2)}%"></u></i></div><div><span>3BET <em>${teamNum(z.threebet)}%</em></span><i><u style="width:${Math.min(100,(+z.threebet||0)*5)}%"></u></i></div><div><span>WWSF <em>${teamNum(z.wwsf)}%</em></span><i><u style="width:${Math.min(100,(+z.wwsf||0)*1.6)}%"></u></i></div></button>`}).join('')||'<p class="muted">Sem dados neste recorte.</p>'}</div></section>
  <div class="team-dashboard-row team-row-3">
    <section class="panel team-manager-plan"><header class="team-section-head"><div><span class="team-section-icon blue">📋</span><h2>Plano de ação do gestor</h2><p class="muted">Priorize o que realmente importa para a evolução do time.</p></div></header><div class="team-action-cards">${priorities.map(x=>{const leaks=(x.snap.leaks||[]).slice(0,3),z=x.snap.stats||{},name=x.member.display_name||x.member.email,initials=name.slice(0,2).toUpperCase();return `<article><header><i>${esc(initials)}</i><span><b>${esc(name)}</b><small>${esc(x.member.role||'player')}</small></span><em>${leaks.filter(l=>l.score>=1.5).length} prioridade(s)</em></header><div class="team-action-list">${leaks.map((l,i)=>`<button class="team-action-item" data-team-player="${esc(x.member.user_id)}"><b>${i+1}</b><span><strong>${esc(l.label)}</strong><small>${teamNum(l.value)}% · ref. ${esc(l.range)}</small></span><em class="${l.score>=1.5?'critical':'important'}">${l.score>=1.5?'ALTA':'MÉDIA'}</em></button>`).join('')||'<span class="muted">Sem foco crítico.</span>'}</div><button class="team-xray-link" data-team-player="${esc(x.member.user_id)}">Ver raio-X completo →</button></article>`}).join('')||'<p class="muted">Sem plano neste recorte.</p>'}</div></section>
    <section class="panel team-insights team-insights-v3"><header class="team-section-head"><div><span class="team-section-icon gold">💡</span><h2>Insights do time</h2><p class="muted">O que os snapshots estão dizendo agora.</p></div></header><div class="team-insight good"><b>↗ Winrate do grupo</b><span>A média atual está em ${avg>=0?'+':''}${teamNum(avg)} bb/100 neste recorte.</span></div><div class="team-insight warn"><b>🎯 Concentração de prioridade</b><span>${priorities[0]?`${esc(priorities[0].member.display_name||priorities[0].member.email)} aparece no topo da fila do gestor.`:'Sem prioridade definida.'}</span></div><div class="team-insight purple"><b>🧠 Foco técnico</b><span>${threeBetCount?`${threeBetCount} sinais estão relacionados a 3Bet.`:'Sem concentração clara em 3Bet.'}</span></div><div class="team-insight blue"><b>👥 Estudo coletivo</b><span>${collective.length?`${esc(collective[0].label)} aparece em ${collective[0].players.size} jogadores.`:'Nenhum leak idêntico aparece em mais de um jogador neste recorte.'}</span></div></section>
  </div>
  <section class="panel team-collective"><header class="team-section-head"><div><span class="team-section-icon purple">👥</span><h2>Leaks coletivos</h2><p class="muted">Problemas que se repetem em mais de um jogador e podem virar estudo em grupo.</p></div><span class="pill">${collective.length} compartilhados</span></header>${collective.length?`<div class="team-collective-grid">${collective.slice(0,6).map((g,i)=>`<article><span class="rank">${i+1}</span><div><b>${esc(g.label)}</b><small>${[...g.players].map(esc).join(' · ')}</small></div><em>${g.players.size} jogadores</em></article>`).join('')}</div>`:'<div class="notice">Neste recorte, nenhum leak com o mesmo nome aparece em dois jogadores. Isso é bom: as prioridades estão mais individualizadas.</div>'}</section>
  </div>`
}
function bindTeamCenterFilters(allRows){
  const render=()=>{const root=document.getElementById('teamCenterRoot');if(!root)return;const filtered=teamFilteredRows(allRows);root.innerHTML=teamcenterHtml(filtered,allRows);bindTeamCenterFilters(allRows);bindTeamXray(filtered)}
  const p=document.getElementById('teamFilterPlayer'),d=document.getElementById('teamFilterPeriod')
  if(p)p.onchange=()=>{teamCenterFilters.player=p.value;render()}
  if(d)d.onchange=()=>{teamCenterFilters.period=d.value;render()}
  const c=document.getElementById('teamClearFilters');if(c)c.onclick=()=>{teamCenterFilters={player:'all',period:'all'};render()}
}
function bindTeamXray(rows){
  const open=id=>{
    const row=rows.find(x=>x.member.user_id===id)||rows[0]
    if(!row||!row.snap)return
    document.body.insertAdjacentHTML('beforeend',teamPlayerXray(row))
    const close=()=>document.getElementById('teamXrayBackdrop')?.remove()
    document.getElementById('teamXrayClose')?.addEventListener('click',close)
    document.getElementById('teamXrayBackdrop')?.addEventListener('click',e=>{if(e.target.id==='teamXrayBackdrop')close()})
  }
  document.querySelectorAll('[data-team-player]').forEach(el=>el.addEventListener('click',()=>open(el.dataset.teamPlayer)))
  const top=document.getElementById('teamOpenTopXray');if(top)top.onclick=()=>open(rows[0]?.member.user_id)
  document.querySelectorAll('[data-team-nav]').forEach(el=>el.addEventListener('click',()=>{const dest=el.dataset.teamNav;if(dest==='players'){document.querySelector('.team-performance-card')?.scrollIntoView({behavior:'smooth',block:'center'});return}if(dest==='hhstats'||dest==='leaks'){route(dest)}}))
}
async function initTeamCenter(){const root=document.getElementById('teamCenterRoot');if(!root)return;const x=await loadTeamIntel();if(!x.manager){root.innerHTML=`<section class="panel"><h2>Área exclusiva do gestor</h2><p class="muted">Seu perfil não possui permissão de owner/manager para visualizar a inteligência da equipe.</p></section>`;return}if(x.error){root.innerHTML=`<section class="panel"><h2>⚠️ Central do Time indisponível</h2><p class="muted">${esc(x.error?.message||String(x.error))}</p></section>`;return}root.innerHTML=teamcenterHtml(teamFilteredRows(x.rows),x.rows);bindTeamCenterFilters(x.rows);bindTeamXray(teamFilteredRows(x.rows))}
