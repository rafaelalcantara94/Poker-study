...
function loginView(){
  app.innerHTML = `<main class="auth"><div class="authbox"><div class="brand">Poker <b>Study</b><small>V10.4.4 • TEAM INTELLIGENCE</small></div>
...
function shell(){
  app.innerHTML=`<div class="app"><aside class="sidebar"><div class="brand">Poker <b>Study</b><small>V10.4.4 • TEAM INTELLIGENCE</small></div><nav class="nav">
...
function createTeamCoachPlan(topic,rows){
  ...
  Object.assign(plan,{topic,title:`Aula do time · ${topic}`,players:affected,duration:50,source:'Central do Time',reviewLabel:topic,recheckAt:teamCoachRecheckDate(14),perPlayer,maxHands,period:teamCenterFilters.period||'all',agenda:[{mins:10,label:'Conceitos e referência do spot'},{mins:15,label:'Comparar padrões dos jogadores afetados'},{mins:20,label:`Replayer coletivo · até ${maxHands} mãos (${per}/jogador)`},{mins:5,label:'Definir tarefa e critério de reavaliação'}],updatedAt:new Date().toISOString()})
...
function teamcenterHtml(rows,allRows=rows){
  ...
  return `<div class="team-page-v3"><div class="team-page-head"><div><h1>👥 Central do Time</h1><p>Inteligência técnica, prioridades e comparação dos jogadores</p></div><div class="team-update"><span>↻</span><small>Última atualização</small><b>${esc(latestText)}</b></div></div><section class="team-hero team-hero-v3"><div class="team-hero-copy"><span class="team-kicker">V10.4.4 · STUDY WORKFLOW FIX</span><h2>Raio-X técnico da equipe</h2><p>Do diagnóstico à ação: clique, investigue e leve o spot para estudo.</p></div><div class="team-hero-quote">“O painel só tem valor<br>se levar a uma decisão.”</div></section>${teamFilterBar(allRows)}
...
function bindTeamActions(rows,allRows){
  ...
  document.querySelectorAll('[data-team-create-plan]').forEach(el=>el.addEventListener('click',()=>{const plan=createTeamCoachPlan(el.dataset.teamCreatePlan,allRows);if(!plan)return;studyToast('✓ Aula criada no Plano de Estudos');setTimeout(()=>route('studyplan'),120)}))
  ...
  if(p==='leaks'||p==='studyplan'){
    ...
    if(p==='studyplan'){
      ...
      document.querySelectorAll('[data-coach-plan-done]').forEach(b=>b.onclick=()=>{
        ...
        if(plan){updateTeamCoachPlan(plan.id,{status:plan.status==='done'?'pending':'done',completedAt:plan.status==='done'?null:new Date().toISOString()});route('studyplan')}
      })
      document.querySelectorAll('[data-coach-plan-delete]').forEach(b=>b.onclick=()=>{
        if(confirm('Excluir esta aula do Plano do Gestor?')){removeTeamCoachPlan(b.dataset.coachPlanDelete);route('studyplan')}
      })
    }
  }
