...
function loginView(){
  app.innerHTML = `<main class="auth"><div class="authbox"><div class="brand">Poker <b>Study</b><small>V10.0.3 • TEAM READY</small></div>
...
function shell(){
  app.innerHTML=`<div class="app"><aside class="sidebar"><div class="brand">Poker <b>Study</b><small>V10.0.3 • TEAM READY</small></div><nav class="nav">
...
function playerReloadHtml(){
  ...
  <div class="reload-site-grid">${RELOAD_SITES.map(s=>`<article class="reload-site-card" data-reload-card="${esc(s.id)}">${siteLogoHtml(s.id)}<b>${esc(s.id)}</b><div class="money-input"><span>R$</span><input inputmode="decimal" data-reload-amount="${esc(s.id)}" placeholder="0,00"></div><input class="reload-item-note" data-reload-note="${esc(s.id)}" maxlength="180" placeholder="Observação deste site (opcional)"></article>`).join('')}</div>
  <label class="reload-note">Observação geral do pedido <small>(opcional)</small><textarea id="reloadNote" maxlength="300" placeholder="Ex.: urgência geral, informação que vale para todo o pedido…"></textarea></label>
  ...
  <div class="reload-player-bottom"><section class="panel"><header><div><h3>Seus pedidos recentes</h3><p class="muted">Acompanhe aprovação e envio.</p></div></header>${ownReq.length?ownReq.map(r=>`<div class="reload-own-request"><div><b>${reloadDate(r.created_at)}</b><span>${r.note?`Geral: ${esc(r.note)}`:'Sem observação geral'}</span></div><div>${(r.reload_items||[]).map(i=>`<span class="reload-own-chip">${esc(siteMeta(i.site).short)} · ${brl(i.amount)} ${reloadStatusBadge(i.status)}${i.item_note?`<small>${esc(i.item_note)}</small>`:''}</span>`).join('')}</div></div>`).join(''):'<p class="muted">Nenhum pedido ainda.</p>'}</section><section class="panel reload-makeup-self"><small>SEU MAKE UP ATUAL</small><strong class="${makeup>0?'bad':'good'}">${brl(makeup)}</strong><span>${makeup>0?'Em Make Up':'Em dia'}</span><p>O Make Up é controlado pelo gestor em um razão separado dos reloads.</p></section></div>`
}
