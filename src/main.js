...
function loginView(){
  app.innerHTML = `<main class="auth"><div class="authbox"><div class="brand">Poker <b>Study</b><small>V9.3 • TRACKER</small></div>
...
function shell(){
  app.innerHTML=`<div class="app"><aside class="sidebar"><div class="brand">Poker <b>Study</b><small>V9.3 • TRACKER</small></div><nav class="nav">
...
function hhStatsViewHtml(facts,totalFacts=hhStatsCache){
  ...
  return `<div class="v7-dashboard">
    <div class="v7-resultbar"><b>${facts.length.toLocaleString('pt-BR')} mãos encontradas</b><span>${breakdown}</span><em>Painel V9.3: Leak Evolution + Review Destinations</em></div>
...
function leakEvolutionHtml(){
  ...
  if(!topics.length)return `<div class="panel leak-evolution-panel"><div class="leak-evo-head"><div><h2>📈 Evolução dos leaks</h2><p class="muted">Confirme um leak ou envie uma mão para o Plano de Estudos para começar o acompanhamento.</p></div><span class="pill">V9.3</span></div></div>`
  if(!facts.length)return `<div class="panel leak-evolution-panel"><div class="leak-evo-head"><div><h2>📈 Evolução dos leaks</h2><p class="muted">Carregando o snapshot do Stats HH para comparar antes e depois…</p></div><span class="pill">V9.3</span></div></div>`
  return `<div class="panel leak-evolution-panel"><div class="leak-evo-head"><div><h2>📈 Evolução dos leaks</h2><p class="muted">Compara as mãos anteriores à confirmação com as novas mãos importadas depois dela. Melhorar significa aproximar-se da faixa de referência — não simplesmente aumentar a stat.</p></div><span class="pill">V9.3</span></div><div class="leak-evo-list">${topics.map(g=>leakEvolutionCard(g,facts)).join('')}</div><div class="notice">O acompanhamento começa na data da primeira confirmação manual do tema. “Amostra insuficiente” evita concluir melhora ou piora cedo demais.</div></div>`
}
