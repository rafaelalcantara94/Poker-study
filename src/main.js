...
function loginView(){
  app.innerHTML = `<main class="auth"><div class="authbox"><div class="brand">Poker <b>Study</b><small>V9.1.2 • TRACKER</small></div>
...
function shell(){
  app.innerHTML=`<div class="app"><aside class="sidebar"><div class="brand">Poker <b>Study</b><small>V9.1.2 • TRACKER</small></div><nav class="nav">
...
function hhStatsViewHtml(facts,totalFacts=hhStatsCache){
  ...
  return `<div class="v7-dashboard">
    <div class="v7-resultbar"><b>${facts.length.toLocaleString('pt-BR')} mãos encontradas</b><span>${breakdown}</span><em>Painel V9.1.2: Study Workflow + Performance</em></div>
...
function studyWorkflowHtml(h){
  ...
  const dest=r.status?`Destino local: ${studyDestinationLabel(r.status)}`:'As revisões desta V9.1.2 ficam neste navegador'
...
function reviewDestinationText(x){return x.sentTo==='leaks'?'✓ Enviado à Central de Leaks':x.sentTo==='plan'?'✓ Enviado ao Plano de Estudos':''}
function reviewCardHtml(x){
  ...
  const sent=!!x.sentTo
  const send=!sent&&x.status==='leak'?`<button class="btn small" data-review-send="leaks" data-review-id="${esc(x.handId)}">🧠 Enviar para Leaks</button>`:!sent&&x.status==='theory'?`<button class="btn small" data-review-send="plan" data-review-id="${esc(x.handId)}">🗓️ Enviar para Plano</button>`:''
  const reclass=!sent?`<select class="review-reclassify" data-review-reclassify="${esc(x.handId)}" title="Reclassificar revisão"><option value="">Reclassificar…</option><option value="correct">✓ Correta</option><option value="doubt">? Dúvida</option><option value="leak">⚠ Possível leak</option><option value="theory">📚 Rever teoria</option></select>`:''
  return `<article class="review-box-card ${esc(x.status)} ${sent?'resolved':''}"><div class="review-box-icon">${studyStatusIcon(x.status)}</div><div class="review-box-main"><div class="review-box-title"><b>${esc(cards)}</b><span>${esc(x.position||'')}${x.stackBb!=null?` · ${esc(String(x.stackBb))}bb`:''}</span></div><p>${esc(x.note||'Sem nota')}</p><small>${esc(x.context||x.source||'Revisão do Replayer')}${when?` · ${esc(when)}`:''}${legacy?' · legado V9.0.1':''}</small>${sent?`<small class="good">${reviewDestinationText(x)}</small>`:''}</div><div class="review-box-actions"><span class="review-box-status ${esc(x.status)}">${sent?reviewDestinationText(x):esc(studyStatusLabel(x.status))}</span><button class="btn small secondary" data-review-open="${esc(x.handId)}" ${can?'':'disabled'}>${can?'🎬 Abrir mão':'Fonte não carregada'}</button>${send}${reclass}<button class="btn small danger" data-review-delete="${esc(x.handId)}">🗑 Excluir</button></div></article>`
}
function reviews(){
  const items=reviewBoxItems(),counts={correct:0,doubt:0,leak:0,theory:0};items.forEach(x=>counts[x.status]=(counts[x.status]||0)+1)
  const pending=items.filter(x=>x.status!=='correct'&&!x.sentTo)
  const active=window.reviewBoxFilter||'attention',filtered=active==='all'?items:active==='attention'?pending:items.filter(x=>x.status===active)
  return `<div class="review-box-summary"><div><h2>📥 Caixa de Revisões</h2><p>Pendências mostra somente o que ainda exige uma decisão sua. Itens enviados ou concluídos continuam preservados em Todas.</p></div><div class="review-box-kpis"><span><b>${pending.length}</b> pendências</span><span><b>${counts.doubt}</b> dúvidas</span><span><b>${counts.leak}</b> possíveis leaks</span><span><b>${counts.theory}</b> teoria</span></div></div><div class="review-box-tabs">${[['attention','Pendências'],['doubt','Dúvidas'],['leak','Possíveis leaks'],['theory','Rever teoria'],['correct','Corretas'],['all','Todas']].map(([k,l])=>`<button class="${active===k?'active':''}" data-review-filter="${k}">${l}</button>`).join('')}</div><div class="review-box-note"><b>Fluxo V9.1.2:</b> reclassifique uma revisão quando sua conclusão mudar. Ao enviar para Leaks/Plano ou marcar como Correta, ela sai de Pendências e permanece no histórico em Todas. Excluir remove definitivamente a revisão local.</div><section class="review-box-list">${filtered.length?filtered.map(reviewCardHtml).join(''):'<div class="panel"><p class="muted">Nenhuma revisão neste filtro.</p></div>'}</section>`
}
...
function sendReviewDestination(id,dest){
  ...
  const label=dest==='leaks'?'Central de Leaks':'Plano de Estudos';if(!confirm(`Enviar esta revisão para ${label}? Ela sairá de Pendências e continuará disponível em Todas.`))return
  r.sentTo=dest;r.sentAt=new Date().toISOString();r.updatedAt=r.sentAt;all[id]=r;localStorage.setItem(STUDY_REVIEW_KEY,JSON.stringify(all));studyToast(`✓ Enviado para ${label}`);refreshReviewsPage()
}
function reclassifyReview(id,status){
  if(!status)return
  const all=studyReviews(),r=all[id];if(!r)return
  r.status=status;r.destination=studyDestinationLabel(status);r.updatedAt=new Date().toISOString();delete r.sentTo;delete r.sentAt;all[id]=r;localStorage.setItem(STUDY_REVIEW_KEY,JSON.stringify(all));studyToast(`${studyStatusIcon(status)} Reclassificada como ${studyStatusLabel(status)}`);refreshReviewsPage()
}
...
function bindReviews(){
  ...
  document.querySelectorAll('[data-review-reclassify]').forEach(s=>s.onchange=()=>reclassifyReview(s.dataset.reviewReclassify,s.value))
