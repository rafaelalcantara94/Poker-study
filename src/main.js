...
const HH_STATS_SNAPSHOT_SCHEMA=4
...
const APP_THEMES=[
  ['classic','Clássico'],['dark-modern','Dark Moderno'],['emerald','Emerald'],['red','Red'],
  ['minimal-premium','Minimalista Premium'],['light-clean','Light Clean'],['purple','Purple'],['gold','Gold'],['neon-future','Neon / Future']
]
const REPLAYER_THEMES=[['classic','Clássico'],['clean-pro','Clean Pro'],['icemizer-pro','Icemizer Pro'],['tv-pro','TV Pro'],['minimal','Minimalista'],['classic-wood','Classic Wood'],['future-neon','Future Neon']]
const prefKey=()=>`paineldr:prefs:${user?.id||'guest'}`
const onboardingKey=()=>`paineldr:onboarding:${user?.id||'guest'}`
function visualPrefs(){try{return {...{appTheme:'classic',replayerTheme:'classic'},...(user?.user_metadata?.paineldr_visual||{}),...JSON.parse(localStorage.getItem(prefKey())||'{}')}}catch{return {...{appTheme:'classic',replayerTheme:'classic'},...(user?.user_metadata?.paineldr_visual||{})}}}
function applyVisualPrefs(p=visualPrefs()){
  document.documentElement.dataset.appTheme=p.appTheme||'classic'
  document.documentElement.dataset.replayerTheme=p.replayerTheme||'classic'
}
function saveVisualPrefs(p){localStorage.setItem(prefKey(),JSON.stringify(p));applyVisualPrefs(p);if(user)supabase.auth.updateUser({data:{paineldr_visual:p}}).then(({data})=>{if(data?.user)user=data.user}).catch(()=>{})}
function themeOptions(items,current,kind){return items.map(([id,label])=>`<label class="theme-choice ${current===id?'selected':''}"><input type="radio" name="${kind}" value="${esc(id)}" ${current===id?'checked':''}><span class="theme-swatch ${kind}-${esc(id)}"></span><b>${esc(label)}</b></label>`).join('')}
function appearance(){const p=visualPrefs();return `<div class="panel appearance-panel"><div class="appearance-head"><div><small>PAINEL DR · APARÊNCIA</small><h2>Escolha o seu visual</h2><p class="muted">As alterações são individuais, instantâneas e reversíveis. Nenhum dado ou funcionalidade é alterado.</p></div><button class="btn secondary" id="resetAppearance">Restaurar padrão</button></div><h3>Tema do App</h3><div class="theme-grid" id="appThemeGrid">${themeOptions(APP_THEMES,p.appTheme,'appTheme')}</div><h3>Estilo do Replayer</h3><div class="theme-grid replay-theme-grid" id="replayerThemeGrid">${themeOptions(REPLAYER_THEMES,p.replayerTheme,'replayerTheme')}</div><div class="notice appearance-tip">✨ Você pode trocar de visual quantas vezes quiser. O Clássico corresponde ao visual validado na V14.5.0.</div></div>`}
function bindAppearance(){
  const sync=()=>{const a=document.querySelector('input[name="appTheme"]:checked')?.value||'classic',r=document.querySelector('input[name="replayerTheme"]:checked')?.value||'classic';saveVisualPrefs({appTheme:a,replayerTheme:r});document.querySelectorAll('.theme-choice').forEach(x=>x.classList.toggle('selected',!!x.querySelector('input:checked')))}
  document.querySelectorAll('.theme-choice input').forEach(x=>x.onchange=sync)
  const reset=document.getElementById('resetAppearance');if(reset)reset.onclick=()=>{saveVisualPrefs({appTheme:'classic',replayerTheme:'classic'});route('appearance')}
}
function onboardingView(){const p=visualPrefs();app.innerHTML=`<main class="onboarding"><section class="onboarding-box"><div class="brand">Painel <b>DR</b><small>SEU JOGO, COM PROPÓSITO</small></div><div class="onboarding-welcome"><small>PRIMEIRO ACESSO</small><h1>Bem-vindo ao Painel DR.</h1><p>Antes de começar, personalize seu ambiente. Você poderá mudar tudo novamente em Configurações.</p></div><div class="onboarding-step"><span>1</span><div><h2>Senha de acesso</h2><p class="muted">Se recebeu uma senha temporária, altere agora. Se já definiu sua própria senha, pode deixar em branco.</p><div class="onboarding-password"><input id="onboardPass" type="password" placeholder="Nova senha (opcional)"><input id="onboardPass2" type="password" placeholder="Confirmar nova senha"></div></div></div><div class="onboarding-step"><span>2</span><div><h2>Tema do App</h2><div class="theme-grid compact">${themeOptions(APP_THEMES,p.appTheme,'appTheme')}</div></div></div><div class="onboarding-step"><span>3</span><div><h2>Estilo do Replayer</h2><div class="theme-grid compact">${themeOptions(REPLAYER_THEMES,p.replayerTheme,'replayerTheme')}</div></div></div><div class="onboarding-actions"><p id="onboardMsg" class="muted"></p><button class="btn" id="finishOnboarding">Entrar no Painel →</button></div></section></main>`;applyVisualPrefs(p);document.querySelectorAll('.theme-choice input').forEach(x=>x.onchange=()=>{const q={appTheme:document.querySelector('input[name="appTheme"]:checked')?.value||'classic',replayerTheme:document.querySelector('input[name="replayerTheme"]:checked')?.value||'classic'};applyVisualPrefs(q);document.querySelectorAll('.theme-choice').forEach(y=>y.classList.toggle('selected',!!y.querySelector('input:checked'))) });finishOnboarding.onclick=async()=>{const a=onboardPass.value,b=onboardPass2.value;if(a||b){if(a.length<6)return onboardMsg.textContent='A nova senha precisa ter pelo menos 6 caracteres.';if(a!==b)return onboardMsg.textContent='As senhas não são iguais.';finishOnboarding.disabled=true;onboardMsg.textContent='Alterando senha...';const {error}=await supabase.auth.updateUser({password:a});if(error){finishOnboarding.disabled=false;return onboardMsg.textContent=error.message}}const q={appTheme:document.querySelector('input[name="appTheme"]:checked')?.value||'classic',replayerTheme:document.querySelector('input[name="replayerTheme"]:checked')?.value||'classic'};saveVisualPrefs(q);localStorage.setItem(onboardingKey(),'done');const {data:metaData}=await supabase.auth.updateUser({data:{paineldr_onboarding:true,paineldr_visual:q}});if(metaData?.user)user=metaData.user;shell()}}
function launchApp(){applyVisualPrefs();if(user?.user_metadata?.paineldr_onboarding!==true&&localStorage.getItem(onboardingKey())!=='done')onboardingView();else shell()}

function loginView(){
  app.innerHTML = `<main class="auth"><div class="authbox"><div class="brand">Painel <b>DR</b><small>V15.0.0 • VISUAL THEMES</small></div>
...
async function loadAccessContext(){
  ...
  }catch(e){console.warn('[Painel DR][Access] fallback player',e);accessCtx={role:'player',canManage:false}}
}
function shell(){
  const managerNav=accessCtx.canManage
  const navItems=[['dashboard','📊 Dashboard'],['analytics','📉 Analytics'],['studies','📚 Estudos'],['hands','🖐️ Mãos'],['replayer','🎬 Replayer'],['reviews','📥 Revisões'],['hhstats','📊 Stats HH'],['reload','💲 Reload / Caixas'],['importer','↥ SharkScope / CSV'],...(managerNav?[['teamcenter','👥 Central do Time'],['leaks','🧠 Central de Leaks']]:[]),['plan','🗓️ Plano de Estudos'],['evolution','🚀 Evolução'],['goals','🎯 Metas'],['reports','📈 Relatórios'],['appearance','⚙️ Aparência']]
  app.innerHTML=`<div class="app"><aside class="sidebar"><div class="brand">Painel <b>DR</b><small>V15.0.0 • VISUAL THEMES</small></div><nav class="nav">
...
function route(p){
  if((p==='teamcenter'||p==='leaks')&&!accessCtx.canManage){p='dashboard';console.warn('[Painel DR][Access] player blocked from manager intelligence route')}
...
const LEAK_CORRECTION_KEY='poker-study-leak-corrections-v141'
function leakCorrections(){try{return JSON.parse(localStorage.getItem(LEAK_CORRECTION_KEY)||'[]')}catch{return []}}
function saveLeakCorrections(rows){localStorage.setItem(LEAK_CORRECTION_KEY,JSON.stringify(rows||[]))}
function leakCorrectionKey(metric,pos,target){return [metric||'',pos||'all',target||'hits'].join('|')}
function activeLeakCorrection(metric,pos,target){const k=leakCorrectionKey(metric,pos,target);return leakCorrections().find(x=>x.key===k&&x.status==='tracking')||null}
function startLeakCorrectionFromSession(session){
  if(!session?.leakMetric)return
  const rows=leakCorrections(),key=leakCorrectionKey(session.leakMetric,session.leakPos,session.leakTarget),old=rows.find(x=>x.key===key&&x.status==='tracking')
  const rec={key,status:'tracking',metric:session.leakMetric,pos:session.leakPos||'all',target:session.leakTarget||'hits',label:session.title||session.leakMetric,startedAt:new Date().toISOString(),baselineValue:+session.leakBaselineValue||0,baselineDen:+session.leakBaselineDen||0,baselineNum:+session.leakBaselineNum||0,room:session.leakRoom||'all',studyId:session.id}
  if(old)Object.assign(old,rec);else rows.unshift(rec)
  saveLeakCorrections(rows.slice(0,80))
}
function leakCorrectionHtml(entry,reviewRows,reviewHits,reviewTarget){
  const c=activeLeakCorrection(entry?.metric,entry?.pos,reviewTarget);if(!c)return ''
  const start=String(c.startedAt||'').slice(0,10),post=(reviewRows||[]).filter(x=>String(x.date||'')>start),hitIds=new Set((reviewHits||[]).map(x=>String(x.handId))),postHits=post.filter(x=>hitIds.has(String(x.handId)))
  const n=post.length,val=n?100*postHits.length/n:null,need=Math.max(0,50-n)
  const state=n<50?'Amostra em formação':(val!=null&&Math.abs(val-c.baselineValue)>=2?'Mudança detectada':'Ainda sem mudança clara')
  return `<section class="v141-correction"><header><div><small>V14.1 · CORREÇÃO EM ACOMPANHAMENTO</small><h3>Antes × Depois</h3></div><span>${esc(state)}</span></header><div class="v141-correction-grid"><div><small>ANTES DO ESTUDO</small><b>${(+c.baselineValue||0).toFixed(1)}%</b><span>${(+c.baselineDen||0).toLocaleString('pt-BR')} oportunidades</span></div><div><small>DEPOIS DO ESTUDO</small><b>${val==null?'—':val.toFixed(1)+'%'}</b><span>${n.toLocaleString('pt-BR')} novas oportunidades</span></div><div><small>STATUS</small><b>${n<50?`faltam ${need}`:'amostra mínima atingida'}</b><span>Início: ${start?start.split('-').reverse().join('/'):'—'}</span></div></div><footer>O Painel DR só compara decisões posteriores ao estudo. O status descreve a amostra; não declara automaticamente que a estratégia está correta.</footer></section>`
}
...
function createTeamCoachPlan(topic,rows){
  ...
  Object.assign(plan,{topic,title:`Aula do time · ${topic}`,players:affected,duration:50,source:'Central do Time',reviewLabel:topic,recheckAt:teamCoachRecheckDate(14),perPlayer:per,maxHands,period:teamCenterUI.getFilters().period||'all',agenda:[{mins:10,label:'Conceitos e referência do spot'},{mins:15,label:'Comparar padrões dos jogadores afetados'},{mins:20,label:`Replayer coletivo · até ${maxHands} mãos (${per}/jogador)`},{mins:5,label:'Definir tarefa e critério de reavaliação'}],updatedAt:new Date().toISOString()})
...
function studySessionReviews(session){
  // V14: sessões criadas diretamente de um leak podem carregar as HHs selecionadas
  // sem exigir que elas tenham sido previamente enviadas para a Caixa de Revisões.
  if(Array.isArray(session?.hhFactIds)&&session.hhFactIds.length){
    const ids=new Set(session.hhFactIds.map(String))
    return (hhStatsCache||[]).filter(x=>ids.has(String(x.handId))).slice(0,20).map(x=>({
      handId:x.handId,heroCards:x.heroCards||[],position:x.position||'',stackBb:x.stack,
      note:session.leakStudyWhy||'Mão selecionada automaticamente pelo diagnóstico do leak.'
    }))
  }
...
async function completeActiveStudySession(){
  ...
  const row={user_id:user.id,course:'Painel DR · Plano',title:`Sessão: ${session.title}`,teacher:'Painel DR',topic:session.title,date:today(),duration,status:'done',tags:['workflow','plano',session.title,`reviewed:${reviewed.length}`,`learning:${reviewedLearnings.length}`].join(','),notes:[notes,...reviewedLearnings.map(([id,v])=>`Aprendizado ${id}: ${String(v).trim()}`),related.length?`Mãos relacionadas: ${related.join(', ')}`:'',reviewed.length?`Mãos revisadas: ${reviewed.join(', ')}`:''].filter(Boolean).join('\n\n')}
  ...
  if(session.leakMetric)startLeakCorrectionFromSession(session)
  saveActiveStudySession(null);await load();studyToast(session.leakMetric?'✓ Estudo concluído · correção agora em acompanhamento':'✓ Estudo concluído e registrado');route('studies')
}
...
async function saveHhStatsSnapshot(facts,html=''){
  const rec={id:HH_SNAPSHOT_KEY,facts,html,updatedAt:new Date().toISOString(),schema:HH_STATS_SNAPSHOT_SCHEMA};hhPersistedSnapshot=rec
...
function heroHandFacts(h){
  ...
  return {
    handId:h.handId,tournamentId:h.tournamentId||'',date:h.dateTime.slice(0,10).replace(/\//g,'-'),time:h.dateTime,game:detectGameType(h),room:h.room||h.site||'',heroNickname:h.hero||'',
...
function v140DidacticLeakHtml(entry,reviewTarget,reviewCount){
  if(!entry)return ''
  const below=entry.bench?.min!=null&&entry.value<entry.bench.min
  const above=entry.bench?.max!=null&&entry.value>entry.bench.max
  const direction=below?'abaixo':above?'acima':'fora'
  const meaning=below
    ? `Você está executando <b>${esc(entry.statLabel)}</b> menos vezes do que a faixa de referência neste recorte.`
    : `Você está executando <b>${esc(entry.statLabel)}</b> mais vezes do que a faixa de referência neste recorte.`
  const investigate=reviewTarget==='misses'
    ? 'O estudo prioriza oportunidades válidas em que a ação não aconteceu. A pergunta é: em quais tipos de mão, stack e configuração você está deixando passar a ação?'
    : 'O estudo prioriza mãos em que a ação aconteceu. A pergunta é: em quais tipos de mão, stack e configuração você pode estar repetindo a ação em excesso?'
  const caution=`A referência (${v75RangeText(entry.bench)}) funciona como sinal de investigação, não como resposta GTO. O objetivo é encontrar padrões nas suas próprias decisões antes de concluir que existe um erro estratégico.`
  return `<section class="v140-didactic"><header><div><small>V14 · DIAGNÓSTICO DIDÁTICO</small><h3>O que este desvio quer dizer?</h3></div><span>${entry.value.toFixed(1)}% · ${direction} da referência</span></header><div class="v140-didactic-grid"><div><b>1 · O sinal</b><p>${meaning} Foram ${entry.den.toLocaleString('pt-BR')} oportunidades na amostra atual.</p></div><div><b>2 · O que investigar</b><p>${investigate}</p></div><div><b>3 · Como estudar</b><p>O Painel DR separou ${reviewCount.toLocaleString('pt-BR')} candidato${reviewCount===1?'':'s'}. Comece por uma sessão curta, tome sua decisão antes de revelar a ação original e registre o aprendizado.</p></div></div><footer><span>⚠ ${caution}</span><button class="btn" id="v140StartLeakStudy">🎓 Estudar este leak</button></footer></section>`
}
function v140StartLeakStudy(entry,reviewRows,replayLabel,reviewTarget){
  if(!entry||!reviewRows?.length)return alert('Não há mãos suficientes neste recorte para iniciar o estudo.')
  const chosen=reviewRows.slice(0,Math.min(15,reviewRows.length))
  saveActiveStudySession({
    id:uid(),title:entry.statLabel,kind:'LEAK STUDY · DIAGNÓSTICO DIDÁTICO',mins:35,
    why:`${entry.group} · ${entry.value.toFixed(1)}% · ref. ${v75RangeText(entry.bench)} · ${entry.den.toLocaleString('pt-BR')} oportunidades`,
    startedAt:Date.now(),notes:'',reviewedHandIds:[],handNotes:{},hhFactIds:chosen.map(x=>String(x.handId)),
    leakMetric:entry.metric,leakPos:entry.pos,leakTarget:reviewTarget,leakStudyWhy:`${replayLabel} · sessão criada automaticamente a partir do diagnóstico.`,
    leakBaselineValue:entry.value,leakBaselineDen:entry.den,leakBaselineNum:entry.num||Math.round(entry.value*entry.den/100),leakRoom:(document.getElementById('hhRoomFilter')?.value||'all')
  })
...
function hhAuditModal(metric,pos,reviewTarget='hits',strategicMode=false){
  ...
  const didactic=v140DidacticLeakHtml(v87Entry,reviewTarget,reviewCount)
  const correction=v87Entry?leakCorrectionHtml(v87Entry,rows,hits,reviewTarget):''
...
const openTeamCollectiveReplayer=(label,room='all')=>reviewPackApi.openTeamCollectiveReplayer(label,room)
const openTeamPlayerReviewPack=(label,userId,playerName='Jogador',room='all')=>reviewPackApi.openTeamPlayerReviewPack(label,userId,playerName,room)
const openTeamAreaCollectiveReplayer=(area,rows,userId=null,room='all')=>reviewPackApi.openTeamAreaCollectiveReplayer(area,rows,userId,room)
...
function roomDiagCounts(facts){
  const out={};for(const f of (facts||[])){const r=String(f?.room||'Unknown');out[r]=(out[r]||0)+1}return out
}
function setRoomDiag(localCounts,payloadRooms,serverRooms,note=''){
  const el=document.getElementById('hhRoomDiag');if(!el)return
  const fmt=o=>Object.entries(o||{}).map(([k,v])=>`${k}: ${Number(v||0).toLocaleString('pt-BR')}`).join(' · ')||'—'
  el.innerHTML=`<b>🧪 Diagnóstico Multi-Room</b><span><strong>Fatos locais:</strong> ${esc(fmt(localCounts))}</span><span><strong>Payload:</strong> ${esc((payloadRooms||[]).join(', ')||'—')}</span><span><strong>Supabase/Central:</strong> ${esc((serverRooms||[]).join(', ')||'—')}</span>${note?`<small>${esc(note)}</small>`:''}`
}
async function readPublishedRoomDiag(teamId){
  try{
    const {data,error}=await supabase.rpc('get_team_hh_intelligence',{p_team:teamId});if(error)throw error
    const row=(data||[]).find(r=>r.user_id===user?.id)
    const st=row?.stats||{}
    return {rooms:Array.isArray(st.roomsAvailable)?st.roomsAvailable:[],breakdown:st.roomBreakdown||{},keys:Object.keys(st)}
  }catch(e){return {rooms:[],breakdown:{},error:e?.message||String(e),keys:[]}}
}
async function publishTeamSnapshot(facts){
  ...
  try{
    ...
    const localCounts=roomDiagCounts(facts)
    setRoomDiag(localCounts,payload.roomsAvailable,[],`antes de publicar · schema local ${HH_STATS_SNAPSHOT_SCHEMA}`)
    console.info('[Painel DR][RoomDiag] publish input',{localCounts,roomsAvailable:payload.roomsAvailable,roomBreakdown:payload.roomBreakdown,pokerIdentities:payload.pokerIdentities})
    ...
    const serverDiag=await readPublishedRoomDiag(teamId)
    setRoomDiag(localCounts,payload.roomsAvailable,serverDiag.rooms,serverDiag.error?`erro lendo retorno: ${serverDiag.error}`:`campos no servidor: ${serverDiag.keys.join(', ')}`)
    console.info('[Painel DR][RoomDiag] server readback',serverDiag)
...
const teamCenterUI=createTeamCenterUI({
  esc,
  route,
  getUser:()=>user,
  getLocalCorrections:()=>leakCorrections(),
...
const teamcenter=()=>teamCenterUI.teamcenter()
const teamFilteredRows=rows=>teamCenterUI.teamFilteredRows(rows)
const teamcenterHtml=(rows,allRows)=>teamCenterUI.teamcenterHtml(rows,allRows)
const bindTeamCenterFilters=allRows=>teamCenterUI.bindTeamCenterFilters(allRows)
const bindTeamActions=(rows,allRows)=>teamCenterUI.bindTeamActions(rows,allRows)
...
teamCenterUI.setRefresh(()=>teamCenterRuntime.init())
...
function leakGoalsHtml(){
  const topics=leakEvolutionTopics(),facts=(hhStatsCache||[]).filter(x=>x.game==='holdem')
  if(!topics.length)return `<div class="panel leak-goals-panel"><div class="leak-goals-head"><div><h2>🎯 Metas automáticas dos leaks</h2><p class="muted">Quando um leak confirmado entra em acompanhamento, o Painel DR cria uma meta ligada à faixa de referência.</p></div><span class="pill">V9.6</span></div><div class="notice">Envie uma revisão para Central de Leaks ou Plano de Estudos para criar a primeira meta automática.</div></div>`
...
function bindPage(p){
  if(p==='appearance'){bindAppearance();return}
  ...
  if(p==='replayer'){
    ...
    const loadText=(text,name='Hand History',refreshSaved=true)=>{hhReplayContext=null;studyReplayContext=null;const hands=parsePokerHistory(text);if(!hands.length)return alert('Não consegui reconhecer uma Hand History compatível neste texto (GGNetwork, PokerStars ou ACR).');replayState={hands,selected:hands[0],step:0,sourceName:name,rawText:String(text||''),speed:1,playing:false,showOpponentCards:false,equilabOpen:false,rangeByHand:{},rangeColor:'blue'};replayWorkspace.innerHTML=replayWorkspaceHtml();bindStatsReturn();v835BindHandClassFilter();v835RenderReplayList();if(document.getElementById('replaySearch'))replaySearch.oninput=v835RenderReplayList;bindReplayStage();if(refreshSaved)renderSavedReplays(loadText)}
...
supabase.auth.onAuthStateChange(async(event,s)=>{if(event==='PASSWORD_RECOVERY'){recoveryMode=true;user=s?.user||null;newPasswordView();return}if(recoveryMode)return;user=s?.user||null;if(user){await load();await loadAccessContext();launchApp()}else loginView()})
...
if(recoveryInUrl){recoveryMode=true;newPasswordView()}else if(user){await load();await loadAccessContext();launchApp()}else loginView()
