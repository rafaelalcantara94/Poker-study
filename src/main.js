...
function loginView(){
  app.innerHTML = `<main class="auth"><div class="authbox"><div class="brand">Poker <b>Study</b><small>V9.5.1 • TRACKER</small></div>
...
function shell(){
  app.innerHTML=`<div class="app"><aside class="sidebar"><div class="brand">Poker <b>Study</b><small>V9.5.1 • TRACKER</small></div><nav class="nav">
...
function studySessionHandRow(x,session){
  const raw=x.heroCards||[],visual=raw.length?raw.map(cardHtml).join(''):'<span class="review-dest-fallback">--</span>',cards=raw.join(' ')||'Mão salva',meta=[x.position,x.stackBb!=null?`${x.stackBb}bb`:null].filter(Boolean).join(' · '),reviewed=(session?.reviewedHandIds||[]).includes(x.handId)
  return `<div class="study-session-hand ${reviewed?'reviewed':''}"><div class="study-session-cards">${visual}</div><div><b>${esc(cards)}</b><small>${esc(meta||reviewTopic(x))}</small><p>${esc(x.note||'Sem nota')}</p></div><div class="study-session-hand-actions"><button class="btn small secondary" data-review-open="${esc(x.handId)}" ${reviewCanOpen(x.handId)?'':'disabled'}>${reviewCanOpen(x.handId)?'🎬 Abrir mão':'Fonte indisponível'}</button><button class="btn small ${reviewed?'good':'secondary'}" data-study-review-hand="${esc(x.handId)}">${reviewed?'✓ Revisada':'Marcar revisada'}</button></div></div>`
}
function studySessionHtml(session){
  ...
  return `<section class="panel study-live-session"><header><div><span class="study-live-kicker">▶ SESSÃO EM ANDAMENTO · V9.5.1</span><h2>${esc(session.title)}</h2><p class="muted">${esc(session.kind)} · bloco sugerido ${session.mins} min · iniciado ${new Date(session.startedAt).toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'})}</p></div><div class="study-live-clock" id="studyLiveClock">${String(mins).padStart(2,'0')}:${String(secs).padStart(2,'0')}</div></header><div class="study-live-grid"><div><h3>Mãos relacionadas ${reviews.length?`<span class="pill">${reviews.length}</span>`:''} ${reviews.length?`<span class="study-reviewed-count">${(session.reviewedHandIds||[]).length}/${reviews.length} revisadas</span>`:''}</h3>${reviews.length?`<div class="study-session-hands">${reviews.map(x=>studySessionHandRow(x,session)).join('')}</div>`:'<div class="notice">Ainda não há mãos associadas diretamente a este tema. Você pode usar a sessão para teoria e registrar a conclusão normalmente.</div>'}</div><div class="study-session-notes"><h3>Anotações / conclusão</h3><textarea id="activeStudyNotes" placeholder="O que ficou claro? Que ajuste você quer levar para as próximas sessões?">${esc(session.notes||'')}</textarea><label>Duração a registrar (min) <input id="activeStudyMinutes" type="number" min="1" placeholder="automático pelo cronômetro"></label><div class="study-session-actions"><button class="btn" id="completeStudySession">✓ Concluir estudo</button><button class="btn secondary" id="cancelStudySession">Cancelar sessão</button></div><small class="muted">Ao concluir, a duração e a data entram no histórico de Estudos e alimentam automaticamente Dashboard e Evolução.</small></div></div></section>`
}
function studies(){
  ...
  return `<div class="panel study-plan-entry"><header><div><h2>🧭 Executar o Plano de Estudos</h2><p class="muted">Escolha um bloco sugerido. O Poker Study abre uma sessão, reúne as mãos relacionadas e registra o tempo quando você concluir.</p></div><span class="pill">V9.5.1</span></header><div class="study-plan-cards">${tasks.map((t,i)=>`<article><small>${esc(t.kind)}</small><h3>${esc(t.title)}</h3><p>${esc(t.why)}</p><footer><b>${t.mins} min</b><button class="btn small" data-study-start="${i}">▶ Iniciar estudo</button></footer></article>`).join('')}</div><button class="btn secondary" id="goStudyPlan">Ver Plano de Estudos completo →</button></div>${history}`
}
function startStudyPlanTask(index){const t=studyPlanTasks()[+index];if(!t)return;saveActiveStudySession({id:uid(),title:t.title,kind:t.kind,mins:t.mins,why:t.why,startedAt:Date.now(),notes:'',reviewedHandIds:[]});route('studies')}
...
async function completeActiveStudySession(){
  ...
  const row={user_id:user.id,course:'Poker Study · Plano',title:`Sessão: ${session.title}`,teacher:'Poker Study',topic:session.title,date:today(),duration,status:'done',tags:['workflow','plano',session.title,`reviewed:${reviewed.length}`].join(','),notes:[notes,related.length?`Mãos relacionadas: ${related.join(', ')}`:'',reviewed.length?`Mãos revisadas: ${reviewed.join(', ')}`:''].filter(Boolean).join('\n\n')}
...
function leakEvolutionHtml(){
  ...
  if(!topics.length)return `<div class="panel leak-evolution-panel"><div class="leak-evo-head"><div><h2>📈 Evolução dos leaks</h2><p class="muted">Confirme um leak ou envie uma mão para o Plano de Estudos para começar o acompanhamento.</p></div><span class="pill">V9.5.1</span></div></div>`
  if(!facts.length)return `<div class="panel leak-evolution-panel"><div class="leak-evo-head"><div><h2>📈 Evolução dos leaks</h2><p class="muted">Carregando o snapshot do Stats HH para comparar antes e depois…</p></div><span class="pill">V9.5.1</span></div></div>`
  return `<div class="panel leak-evolution-panel"><div class="leak-evo-head"><div><h2>📈 Evolução dos leaks</h2><p class="muted">Compara as mãos anteriores à confirmação com as novas mãos importadas depois dela. Melhorar significa aproximar-se da faixa de referência — não simplesmente aumentar a stat.</p></div><span class="pill">V9.5.1</span></div><div class="leak-evo-list">${topics.map(g=>leakEvolutionCard(g,facts)).join('')}</div><div class="notice">O acompanhamento começa na data da primeira confirmação manual do tema. “Amostra insuficiente” evita concluir melhora ou piora cedo demais.</div></div>`
}
...
function leakGoalsHtml(){
  const topics=leakEvolutionTopics(),facts=(hhStatsCache||[]).filter(x=>x.game==='holdem')
  if(!topics.length)return `<div class="panel leak-goals-panel"><div class="leak-goals-head"><div><h2>🎯 Metas automáticas dos leaks</h2><p class="muted">Quando um leak confirmado entra em acompanhamento, o Poker Study cria uma meta ligada à faixa de referência.</p></div><span class="pill">V9.5.1</span></div><div class="notice">Envie uma revisão para Central de Leaks ou Plano de Estudos para criar a primeira meta automática.</div></div>`
  if(!facts.length)return `<div class="panel leak-goals-panel"><div class="leak-goals-head"><div><h2>🎯 Metas automáticas dos leaks</h2><p class="muted">Carregando o snapshot do Stats HH para calcular as metas…</p></div><span class="pill">V9.5.1</span></div></div>`
  return `<div class="panel leak-goals-panel"><div class="leak-goals-head"><div><h2>🎯 Metas automáticas dos leaks</h2><p class="muted">Metas geradas a partir dos leaks que você confirmou. O objetivo é entrar — ou permanecer — na faixa de referência, nunca simplesmente aumentar uma stat.</p></div><span class="pill">V9.5.1</span></div><div class="leak-goals-list">${topics.map(g=>leakGoalCard(g,facts)).join('')}</div></div>`
}
